...
MVPでは `owner` / `staff` の2権限を使い、公開デモ前の最小制御としてマスタ管理操作をownerに限定する。

staffはレシピ・材料・カテゴリ・単位の参照、Prep Todayの仕込みタスク操作、BoardMemo操作、自分の表示名編集を行える。レシピ・材料・カテゴリ・単位・店舗情報の作成/編集はownerのみ行える。
