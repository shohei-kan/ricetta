...
公開デモでは、仕込み用Recipe「トマトソース」をIngredient「トマトソース」として登録し、販売商品Recipe「カポナータ」の材料に600g使用する例を用意します。
...
Prep Todayでは `archived_at=null` の未チェックメモを作成日に関係なく表示し続けます。チェック操作で `archived_at=now` にし、チェックした当日中は同じメモカード内の「チェック済み」エリアに薄く表示します。昨日以前にチェック済みのメモは通常表示から外します。

チェック済みメモを再度クリックすると `archived_at=null` に戻し、未チェックメモとして表示します。
